...
from ..models.wan_3dcache_adapter import Cache3dAdapter
...
class WanVideo3DCacheInferPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.cache_adaptor: Cache3dAdapter = None
        ...
        self.units = [
            ...
            WanVideoUnit_CacheImageEmbedderTestVAE(),
    ...
    def load_lora_from_dict(self, module, params_dict, alpha=1):
        loader = GeneralLoRALoader(torch_dtype=self.torch_dtype, device=self.device)
        loader.load(module, params_dict, alpha=alpha)

    def init_cache_adaptor(self, input_dim=64, out_dim=1536):
        patch_size = self.dit.patch_size   # [1, 2, 2]
        self.cache_adaptor = Cache3dAdapter(input_dim, out_dim, patch_size[1:], patch_size[1:])
        self.cache_adaptor.train()
        self.cache_adaptor.requires_grad_(True)
    
    def set_cache_adaptor(self, params_dict, input_dim=64, out_dim=1536):
        patch_size = self.dit.patch_size   # [1, 2, 2]
        self.cache_adaptor = Cache3dAdapter(input_dim, out_dim, patch_size[1:], patch_size[1:])
        self.cache_adaptor.load_state_dict(params_dict)
        self.cache_adaptor.eval()
        self.cache_adaptor.requires_grad_(False)
    ...
    def from_pretrained(
        ...
        # Initialize pipeline
        pipe = WanVideo3DCacheInferPipeline(device=device, torch_dtype=torch_dtype)
    ...
    def __call__(
        ...
        inputs_shared = {
            "target_w2c": dataset_info["target_w2c"],
            "target_intrinsic": dataset_info["target_intrinsic"],
            "cache_frames": dataset_info["cache_frames"],
            "cache_w2cs": dataset_info["cache_w2cs"],
            "cache_intrinsics": dataset_info["cache_intrinsics"],
            "cache_depths": dataset_info["cache_depths"],
            "cache_index": dataset_info["cache_index"],
            "height": dataset_info["height"],
            "width": dataset_info["width"],
            "num_frames": dataset_info["num_frames"],
            "input_image": dataset_info["first_frame"],
            "end_image": end_image,
        ...
        for unit in self.units:
            ...
            # WanVideoUnit_3DCacheRenderTestPipe        render_control_latents # torch.Size([1, 64, 21, 60, 80])
            # WanVideoUnit_NoiseInitializer             噪声 torch.Size([1, 16, 21, 60, 80]))  填充位置 inputs_shared
            # WanVideoUnit_PromptEmbedder               torch.Size([1, 512, 4096]) 2个 positive & negative 填充在不同的input里面
            # WanVideoUnit_S2V                          不进入
            # WanVideoUnit_InputVideoEmbedder           latent就是噪声  torch.Size([1, 16, 21, 60, 80])  
            # WanVideoUnit_CacheImageEmbedderTestVAE    对指定的cache frame vae处理   torch.Size([1, 20, 21, 60, 80])   特征20 时间21 填充位置 inputs_shared
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            render_control_latents = inputs_shared["render_control_latents"]             # torch.Size([1, 64, 21, 60, 80])
            self.cache_adaptor.to(self.device, dtype=self.torch_dtype)
            inputs_shared["render_latents_feat"] = self.cache_adaptor(render_control_latents)   # torch.Size([1, 1536, 21, 30, 40])

            # Inference
            noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep)     # 去噪
            if cfg_scale != 1.0:
                if cfg_merge:
...
class WanVideoUnit_3DCacheRenderTestPipe(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, 
                      cache_frames, cache_depths, cache_w2cs, cache_intrinsics, target_w2c, target_intrinsic, tiled, tile_size, tile_stride):
        ...
        return {"render_control_latents": render_control_latents}
    ...
    def debug_show(self, render_imgs, render_masks):
        ...
        import imageio
        imageio.mimsave(f"/root/render.gif", view_images, fps=10)
...
class WanVideoUnit_ShapeChecker(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("height", "width", "num_frames"))    # 定义好要从input dict中获取哪些参数

    def process(self, pipe: WanVideo3DCacheInferPipeline, height, width, num_frames):
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
        length = (num_frames - 1) // 4 + 1
        if vace_reference_image is not None:
            length += 1
        shape = (1, pipe.vae.model.z_dim, length, height // pipe.vae.upsampling_factor, width // pipe.vae.upsampling_factor)
        noise = pipe.generate_noise(shape, seed=seed, rand_device=rand_device)      # noise torch.Size([1, 16, 21, 60, 80])
        if vace_reference_image is not None:
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        if input_video is None:
            return {"latents": noise}
        pipe.load_models_to_device(["vae"])
        
        # input_video (81, 480, 640, 3)
        input_video = pipe.preprocess_video(input_video)                    # torch.Size([1, 3, 81, 480, 640])
        # torch.Size([1, 16, 21, 60, 80])
        ...
        if pipe.scheduler.training:
            return {"latents": noise, "input_latents": input_latents}  # torch.Size([1, 16, 21, 60, 80]) torch.Size([1, 16, 21, 60, 80])
        else:
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, prompt, positive) -> dict:
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, input_image, end_image, height, width):
...
class WanVideoUnit_CacheImageEmbedderTestVAE(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("cache_frames", "num_frames", "height", "width", "cache_index", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideo3DCacheInferPipeline, cache_frames, num_frames, height, width, cache_index, tiled, tile_size, tile_stride):
        if not pipe.dit.require_vae_embedding:
            return {}
        
        # init
        vae_input = torch.zeros(num_frames, 3, height, width)     # torch.Size([N, 3, 480, 640]) 
        mask = torch.zeros(num_frames, height//8, width//8)       # torch.Size([N, 60, 80])

        for i in range(len(cache_index)):
            cache_i = cache_index[i]
            mask[cache_i] = 1
            cache_img = cache_frames[i]
            cache_img = pipe.preprocess_image(cache_img)[0]     # H, W, 3 -> 3, H, W
            vae_input[cache_i] = cache_img

        mask = mask.unsqueeze(0).to(pipe.device)   # (1, N, h, w)
        vae_input = vae_input.permute(1, 0, 2, 3)  # (3, N, H, W)
        ...
        mask = mask.transpose(1, 2)[0]             # torch.Size([4, 21, 60, 80])

        pipe.load_models_to_device(["vae"])
        y = pipe.vae.encode([vae_input.to(dtype=pipe.torch_dtype, device=pipe.device)], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)[0]
        y = y.to(dtype=pipe.torch_dtype, device=pipe.device)      # torch.Size([16, 21, 60, 80])  vae_input经过vae以后   特征维度=16  时间维度压缩维度21
        y = torch.concat([mask, y])
        y = y.unsqueeze(0)
        y = y.to(dtype=pipe.torch_dtype, device=pipe.device)      # torch.Size([1, 20, 21, 60, 80])
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, input_image, latents, height, width, tiled, tile_size, tile_stride):
...
class WanVideoUnit_FunControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, control_video, num_frames, height, width, tiled, tile_size, tile_stride, clip_feature, y, latents):
...
class WanVideoUnit_FunReference(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, reference_image, height, width):
...
class WanVideoUnit_FunCameraControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, height, width, num_frames, camera_control_direction, camera_control_speed, camera_control_origin, latents, input_image, tiled, tile_size, tile_stride):
...
class WanVideoUnit_SpeedControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, motion_bucket_id):
...
class WanVideoUnit_VACE(PipelineUnit):
    ...
    def process(
        self,
        pipe: WanVideo3DCacheInferPipeline,
...
class WanVideoUnit_TeaCache(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, num_inference_steps, tea_cache_l1_thresh, tea_cache_model_id):
...
class WanVideoUnit_CfgMerger(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, inputs_shared, inputs_posi, inputs_nega):
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_audio(self, pipe: WanVideo3DCacheInferPipeline, input_audio, audio_sample_rate, num_frames, fps=16, audio_embeds=None, return_all=False):
    ...
    def process_motion_latents(self, pipe: WanVideo3DCacheInferPipeline, height, width, tiled, tile_size, tile_stride, motion_video=None):
        pipe.load_models_to_device(["vae"])
        motion_frames = 73
        kwargs = {}
    ...
    def process_pose_cond(self, pipe: WanVideo3DCacheInferPipeline, s2v_pose_video, num_frames, height, width, tiled, tile_size, tile_stride, s2v_pose_latents=None, num_repeats=1, return_all=False):
    ...
    @staticmethod
    def pre_calculate_audio_pose(pipe: WanVideo3DCacheInferPipeline, input_audio=None, audio_sample_rate=16000, s2v_pose_video=None, num_frames=81, height=448, width=832, fps=16, tiled=True, tile_size=(30, 52), tile_stride=(15, 26)):
...
class WanVideoPostUnit_S2V(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheInferPipeline, latents, motion_latents, drop_motion_frames):
...
def model_fn_wan_video(
    ...
    render_latents_feat: torch.Tensor = None,
    **kwargs,
):
    ...
    x, (f, h, w) = dit.patchify_control(x, render_latents_feat)     # torch.Size([1, 1536, 21, 30, 40])
