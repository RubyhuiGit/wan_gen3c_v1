...
class WanVideo3DCachePipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            WanVideoUnit_CacheImageEmbedderVAE(),
    ...
    def training_loss(self, **inputs):
        ...
        inputs["latents"] = self.scheduler.add_noise(inputs["input_latents"], inputs["noise"], timestep)      # torch.Size([1, 16, 21, 60, 80]) 
        training_target = self.scheduler.training_target(inputs["input_latents"], inputs["noise"], timestep)

        render_control_latents = inputs["render_control_latents"]                                             # torch.Size([1, 64, 21, 60, 80])
        self.cache_adaptor.to(self.device, dtype=self.torch_dtype)
        inputs["render_latents_feat"] = self.cache_adaptor(render_control_latents)                            # torch.Size([1, 1536, 21, 30, 40])
...
class WanVideoUnit_3DCacheRender(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("input_video", "input_w2c", "input_intrinsics", "input_depth", "cache_index", "tiled", "tile_size", "tile_stride"))

    def process(self, pipe: WanVideo3DCachePipeline, 
                      input_video, 
                      input_w2c, 
                      input_intrinsics, 
                      input_depth, 
    ...
    def debug_show(self, input_video, render_imgs, render_masks):
        cur_imgs = input_video.numpy().astype(np.uint8)
        import imageio
        imageio.mimsave(f"/root/origin_img.gif", cur_imgs, fps=10)
...
class WanVideoUnit_ShapeChecker(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, height, width, num_frames):
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, prompt, positive) -> dict:
...
class WanVideoUnit_ImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, input_image, end_image, height, width):
...
class WanVideoUnit_CacheImageEmbedderVAE(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video", "num_frames", "height", "width", "cache_index", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideo3DCachePipeline, input_video, num_frames, height, width, cache_index, tiled, tile_size, tile_stride):
        ...
        for cache_i in cache_index:
            mask[cache_i] = 1
            cache_image = input_video[cache_i]
            cache_image = pipe.preprocess_image(cache_image)[0]     # H, W, 3 -> 3, H, W   -1, 1
            vae_input[cache_i] = cache_image
...
class WanVideoUnit_ImageEmbedderVAE(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
        if input_image is None or not pipe.dit.require_vae_embedding:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        image = pipe.preprocess_image(input_image.resize((width, height))).to(pipe.device)     # torch.Size([1, 3, 480, 832])  -1,1
        msk = torch.ones(1, num_frames, height//8, width//8, device=pipe.device)
        msk[:, 1:] = 0        # torch.Size([1, 81, 60, 104])
        if end_image is not None:
            end_image = pipe.preprocess_image(end_image.resize((width, height))).to(pipe.device)
            vae_input = torch.concat([image.transpose(0,1), torch.zeros(3, num_frames-2, height, width).to(image.device), end_image.transpose(0,1)],dim=1)
            msk[:, -1:] = 1
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, input_image, latents, height, width, tiled, tile_size, tile_stride):
...
class WanVideoUnit_FunControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, control_video, num_frames, height, width, tiled, tile_size, tile_stride, clip_feature, y, latents):
...
class WanVideoUnit_FunReference(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, reference_image, height, width):
...
class WanVideoUnit_FunCameraControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, height, width, num_frames, camera_control_direction, camera_control_speed, camera_control_origin, latents, input_image, tiled, tile_size, tile_stride):
...
class WanVideoUnit_SpeedControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, motion_bucket_id):
...
class WanVideoUnit_TeaCache(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, num_inference_steps, tea_cache_l1_thresh, tea_cache_model_id):
...
class WanVideoUnit_CfgMerger(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, inputs_shared, inputs_posi, inputs_nega):
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_audio(self, pipe: WanVideo3DCachePipeline, input_audio, audio_sample_rate, num_frames, fps=16, audio_embeds=None, return_all=False):
    ...
    def process_motion_latents(self, pipe: WanVideo3DCachePipeline, height, width, tiled, tile_size, tile_stride, motion_video=None):
        pipe.load_models_to_device(["vae"])
        motion_frames = 73
        kwargs = {}
    ...
    def process_pose_cond(self, pipe: WanVideo3DCachePipeline, s2v_pose_video, num_frames, height, width, tiled, tile_size, tile_stride, s2v_pose_latents=None, num_repeats=1, return_all=False):
    ...
    @staticmethod
    def pre_calculate_audio_pose(pipe: WanVideo3DCachePipeline, input_audio=None, audio_sample_rate=16000, s2v_pose_video=None, num_frames=81, height=448, width=832, fps=16, tiled=True, tile_size=(30, 52), tile_stride=(15, 26)):
...
class WanVideoPostUnit_S2V(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCachePipeline, latents, motion_latents, drop_motion_frames):
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # Add camera control
    if render_latents_feat is None:
